...
describe("application resource graph", () => {
  it("preserves production identities and provisions auth before the Worker without live credentials", async () => {
  ...
  it("runs everything in one Worker: HTTP, email, the index consumer, and the store's alarm", async () => {
    ...
    expect(await resolveGraphValue(stack.resources["App/MailIndexConsumer"]?.Props)).toMatchObject({
      queueId: "index-id",
      scriptName: "app-test",
      settings: { batchSize: 1, maxConcurrency: 1 },
    });
    const groups = (await resolveGraphValue(stack.bindings.App)) ?? [];
