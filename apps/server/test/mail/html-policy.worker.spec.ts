...
describe("MailHtmlPolicy storage sanitizer", () => {
  it.effect(
    "preserves semantic and table markup while removing comments and non-policy attributes",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("drops raw-text, foreign, and template elements with their content", () =>
    Effect.gen(function* () {
      const dropped = [
        "script",
        "style",
      ...
      const result = yield* sanitize(`<p>before</p>${hostile}<p>after</p>`);
      expect(result.body).toBe("<p>before</p><p>after</p>");
    }),
  );

  it.effect("unwraps unknown and legacy containers so their text survives", () =>
    Effect.gen(function* () {
      const result = yield* sanitize(
  ...
  it.effect("keeps the 12 sectioning/legacy elements without attributes", () =>
    Effect.gen(function* () {
  ...
  it.effect("sanitizes templates nested in SVG/MathML", () =>
    Effect.gen(function* () {
      const result = yield* sanitize(
        "<p>before</p><svg><template><p>x</p></template></svg><math><template></template></math><p>after</p>",
      );
      expect(result.body).toBe("<p>before</p><p>x</p><p>after</p>");
    }),
  );

  it.effect("removes void resources, event handlers, and alternate resource attributes", () =>
    Effect.gen(function* () {
      const result = yield* sanitize(
  ...
  it.effect("repairs malformed fragments without admitting removed content", () =>
    Effect.gen(function* () {
      const result = yield* sanitize(
        '<p title="ok"><b>bold<img src="javascript:alert(1)"><script>bad',
      );
  ...
  it.effect("unwraps document containers and removes the complete head subtree", () =>
    Effect.gen(function* () {
      const result = yield* sanitize(
  ...
  it.effect("retains and canonically generates the bounded presentation property families", () =>
    Effect.gen(function* () {
      const result = yield* sanitize(
  ...
  it.effect("drops every denied property while retaining independent safe declarations", () =>
    Effect.gen(function* () {
      const denied = [
      ...
      const result = yield* sanitize(`<p style="color: red;${denied};padding: 2px">safe</p>`);
  ...
  it.effect("rejects lexer-gap values while retaining independent safe declarations", () =>
    Effect.gen(function* () {
      const result = yield* sanitize(
  ...
  it.effect(
    "rejects important, resource functions, variables, expressions, and unclassified syntax",
    () =>
      Effect.gen(function* () {
        const result = yield* sanitize(
  ...
  it.effect(
    "rejects imports and decoded CSS URL spellings without discarding independent presentation",
    () =>
      Effect.gen(function* () {
        const result = yield* sanitize(
        ...
        expect(styleDeclarations(attribute(paragraphs[1], "style") ?? "").get("color")).toBe(
          "blue",
        );
        expect(styleDeclarations(attribute(paragraphs[2], "style") ?? "").get("padding")).toBe(
          "1px",
        );
        expect(result.body).not.toContain("tracker.test");
        expect(result.body).not.toContain("url(");
  ...
  it.effect("accepts exactly 8 KiB and rejects larger encoded declaration lists", () =>
    Effect.gen(function* () {
  ...
  it.effect(
    "preserves only canonical credential-free absolute HTTPS anchors with forced defenses",
    () =>
      Effect.gen(function* () {
        const result = yield* sanitize(
  ...
  it.effect("stores canonical HTTPS remote sources only as inert sanitizer-owned metadata", () =>
    Effect.gen(function* () {
      const result = yield* sanitize(
  ...
  it.effect(
    "strips caller-supplied remote metadata instead of treating it as sanitizer-owned",
    () =>
      Effect.gen(function* () {
        const result = yield* sanitize(
...
describe("MailHtmlPolicy parser resource budgets", () => {
  it.effect("accepts the UTF-8 input byte boundary and rejects one byte over it", () =>
    Effect.gen(function* () {
      yield* sanitize("x".repeat(MAIL_HTML_PARSE_LIMITS.inputBytes - 1));
      yield* sanitize("x".repeat(MAIL_HTML_PARSE_LIMITS.inputBytes));
      yield* expectResourceExhaustion(sanitize("x".repeat(MAIL_HTML_PARSE_LIMITS.inputBytes + 1)));
      yield* expectResourceExhaustion(
        sanitize("é".repeat(MAIL_HTML_PARSE_LIMITS.inputBytes / 2 + 1)),
      );
    }),
  );

  it.effect("guards the exact parse5 allocation boundary and resets counters for each parse", () =>
    Effect.gen(function* () {
  ...
  it.effect("bounds open elements and accepts a final tree exactly 128 nodes deep", () =>
    Effect.gen(function* () {
      ...
      yield* sanitize(below);
      yield* sanitize(at);
      yield* expectResourceExhaustion(sanitize(over));
    }),
  );
  ...
  it.effect("enforces per-element attributes for parsed and adopted attributes", () =>
    Effect.gen(function* () {
      ...
      expect((yield* sanitize(`<p ${below}>below</p>`)).body).toBe("<p>below</p>");
      expect((yield* sanitize(`<p ${at}>at</p>`)).body).toBe("<p>at</p>");
      yield* expectResourceExhaustion(sanitize(`<p ${over}>over</p>`));
      expect((yield* sanitize(`<html ${at}><p>adopted</p></html>`)).body).toBe("<p>adopted</p>");
      yield* expectResourceExhaustion(sanitize(`<html ${over}><p>adopted</p></html>`));
    }),
  );
  ...
  it.effect("charges only attributes newly appended by parse5 adoption", () =>
    Effect.gen(function* () {
      const repeatedDuplicate = "<html data-same=x>".repeat(
        MAIL_HTML_PARSE_LIMITS.admittedAttributes + 1,
      );
      const stored = yield* sanitize(`${repeatedDuplicate}<p>kept</p>`);
      expect(stored.body).toBe("<p>kept</p>");
    }),
  );

  it.effect("enforces the aggregate admitted-attribute boundary", () =>
    Effect.gen(function* () {
      ...
      yield* sanitize(at.slice(0, -fullElement.length));
      yield* sanitize(at);
      yield* expectResourceExhaustion(sanitize(`${at}<p data-extra=x></p>`));
    }),
  );

  it.effect(
    "keeps malformed formatting, foster parenting, templates, and foreign content safe",
    () =>
      Effect.gen(function* () {
        const fixtures = [
        ...
        const bodies = yield* Effect.forEach(fixtures, (fixture) =>
          sanitize(fixture).pipe(Effect.flatMap((stored) => materialize(stored.body))),
        );
        expect(bodies).toHaveLength(fixtures.length);
      }),
  );

  it.effect("charges elements cloned by the adoption-agency repair path", () =>
    Effect.gen(function* () {
      const repairedFormattingUnit = '<b><i title="clone">x</b>y</i>';
      yield* sanitize(repairedFormattingUnit.repeat(3_999));
      yield* expectResourceExhaustion(sanitize(repairedFormattingUnit.repeat(4_000)));
    }),
  );

  it.effect("rejects the 240,000-element amplification probe as resource exhaustion", () =>
    expectResourceExhaustion(sanitize("<b></b>".repeat(240_000))),
  );

  it.effect("accepts canonical output at the aggregate limit and materializes it", () =>
    Effect.gen(function* () {
      const atCanonicalLimit = resourceExpandingAnchor().repeat(8_000);
      const stored = yield* sanitize(atCanonicalLimit);
      expect(stored.body).toContain('rel="noopener noreferrer nofollow"');
      expect(yield* materialize(stored.body)).toContain("open");
    }),
  );

  it.effect("rejects sanitizer-generated attribute expansion before returning stored HTML", () =>
    expectResourceExhaustion(sanitize(resourceExpandingAnchor().repeat(9_300))),
  );

  it.effect(
    "rejects sanitizer-generated canonical byte expansion before returning stored HTML",
    () =>
      Effect.gen(function* () {
        ...
        yield* expectResourceExhaustion(sanitize(source));
      }),
  );
});

describe("MailHtmlPolicy remote image materializer", () => {
  it.effect("re-sanitizes stored HTML before activating remote images", () =>
    Effect.gen(function* () {
      ...
      const bodies = yield* Effect.forEach(cases, ([body]) => materialize(body));
      expect(bodies).toEqual(cases.map(([, expected]) => expected));
    }),
  );

  it.effect("regenerates stored inline styles canonically", () =>
    Effect.gen(function* () {
  ...
  it.effect("activates only cross-origin HTTPS metadata and sets no-referrer", () =>
    Effect.gen(function* () {
      ...
      const body = yield* materialize(stored.body);
      ...
      expect(attribute(cross, "data-umail-remote-src")).toBe("https://tracker.test/pixel?a=1&b=2");
  ...
  it.effect("changes nothing but activated image attributes in a rich stored template", () =>
    Effect.gen(function* () {
      const stored = yield* sanitize(
      ...
      expect(yield* materialize(stored.body)).toBe(
        stored.body.replace(
...
const expectResourceExhaustion = Effect.fn("expectResourceExhaustion")(function* (
  operation: Effect.Effect<StoredMailHtml, MailHtmlPolicyError>,
) {
  expect(yield* Effect.flip(operation)).toMatchObject({
