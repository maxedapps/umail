...
import { OPERATOR_PASSWORD, TEST_SITE, createWorld, readText } from "./world.ts";

const responseHtml = Effect.fn("responseHtml")(function* (
  response: HttpServerResponse.HttpServerResponse,
) {
  const webResponse = HttpServerResponse.toWeb(response);
  return { response: webResponse, html: yield* readText(webResponse) };
});
...
describe("auth HTML pages", () => {
  it.effect(
    "renders a secure, labelled login flow through the existing Better Auth endpoints",
    () =>
      Effect.gen(function* () {
        const { response, html } = yield* responseHtml(loginPageResponse());
  ...
  it.effect("keeps credentials out of the URL when the login script cannot run", () =>
    Effect.gen(function* () {
      const { html } = yield* responseHtml(loginPageResponse());
  ...
  it.effect("renders semantic consent controls while preserving the OAuth query and endpoint", () =>
    Effect.gen(function* () {
      const { response, html } = yield* responseHtml(consentPageResponse());
      const nonce = nonceFromCsp(response);
  ...
  it.effect("serves the styled pages from the existing GET routes", () =>
    Effect.gen(function* () {
      const world = yield* createWorld();
      const login = yield* world.request("http://umail.test/login?oauth_request=preserved");
      const consent = yield* world.request(
        "http://umail.test/consent?client_id=client-1&scope=openid%20offline_access",
      );

      expect(login.status).toBe(200);
      expect(yield* readText(login)).toContain('id="login-form"');
      expect(consent.status).toBe(200);
      expect(yield* readText(consent)).toContain('id="consent-form"');
    }),
  );
