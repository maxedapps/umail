...
describe("human pages in Chromium", () => {
  it("executes the nonce-authorized login and consent scripts through their first-party endpoints", async () => {
    const consent = await observe("consent");
    const login = await observe("login");
  ...
  it("keeps hostile approval metadata inert and exposes one keyboard-usable native decision form", async () => {
  ...
  it.each([
    ...
    "renders %s as an action-free terminal page",
    async (fixture, status, expectedMeaning) => {
      const terminal = await observe(fixture);
  ...
  it("keeps ordinary next returns out of oauth_query and ignores hostile destinations", async () => {
    const clients = await observe("login", {
      width: 1280,
      height: 900,
      colorScheme: "light",
      search: "next=/clients",
  ...
  it("avoids horizontal overflow at 320px and computes distinct light and dark presentation", async () => {
    const mobileFixtures = ["login", "consent", "pending", "accepted"] as const;
    for (const fixture of mobileFixtures) {
      const mobile = await observe(fixture, { width: 320, height: 900, colorScheme: "light" });
    ...
    const light = await observe("pending", {
      width: 1280,
      height: 900,
      colorScheme: "light",
    });
    const dark = await observe("pending", {
      width: 1280,
      height: 900,
      colorScheme: "dark",
    });
...
async function observe(
  ...
  const raw = await commands.observeHumanPage(visit);
  const decoded = Schema.decodeResult(HumanPageBrowserObservationSchema)(raw);
  if (Result.isFailure(decoded)) {
    throw new Error(`Invalid human-page browser observation: ${decoded.failure.message}`);
  }
