...
async function consent(accept) {
  ...
  try {
    const response = await fetch("/api/auth/oauth2/consent", {
      ...
      headers: { "content-type": "application/json", accept: "application/json" },
      body: JSON.stringify(
        oauthQuery === null ? { accept } : { accept, oauth_query: oauthQuery },
      ),
    });
    if (!response.ok) {
...
export function consentPageResponse() {
  const page = renderHumanPageInternal({
    ...
    document: {
      ...
      description:
        "A client is asking to use your mailbox through AgentMail. Confirm the client and requested scope before deciding.",
