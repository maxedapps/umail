...
export async function deriveApprovalToken(
  ...
  const mac = await crypto.subtle.sign("HMAC", hmacKey, new TextEncoder().encode(approvalId));
...
export async function newApprovalCapability(
  key: NotificationKey,
  nowIso: string,
): Promise<ApprovalCapabilityWrite> {
  const approvalId = crypto.randomUUID();
  const token = await deriveApprovalToken(key, approvalId);
  return {
    approvalId,
